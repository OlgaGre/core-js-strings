...
function formatTime(minutes, seconds) {
  ...
  const strSex = seconds.toString();

  return `${strMin.padStart(2, '0')}:${strSex.padStart(2, '0')}`;
}
...
function isPalindrome(str) {
  ...
  return (
    str.replace(re, '').toUpperCase() ===
    str.replace(re, '').toUpperCase().split('').reverse().join('')
  );
}
...
reverseWords('UPPERCASE lowercase');
/**
